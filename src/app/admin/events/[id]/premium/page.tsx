...
export default function PremiumEventEditorPage({ params }: { params: { id: string } }) {
    ...
    <div className="p-6 sm:p-8 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <section className="card-elegant p-6 space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <label>
              <span className="text-xs font-semibold text-elegant/50 uppercase tracking-wider flex gap-1 items-center"><Type size={13} /> Font Judul</span>
              <select value={settings.headingFont} onChange={(e) => updateSetting('headingFont', e.target.value)} className="input-elegant mt-1">
                {fonts.map((font) => <option key={font}>{font}</option>)}
              </select>
            </label>
            <label>
              <span className="text-xs font-semibold text-elegant/50 uppercase tracking-wider flex gap-1 items-center"><Type size={13} /> Font Isi</span>
              <select value={settings.bodyFont} onChange={(e) => updateSetting('bodyFont', e.target.value)} className="input-elegant mt-1">
                {fonts.map((font) => <option key={font}>{font}</option>)}
              </select>
            </label>
          </div>
          ...
          <label className="block">
            ...
            <select value={event.theme_slug || 'classic-elegant'} onChange={(e) => setEvent({ ...event, theme_slug: e.target.value })} className="input-elegant mt-1">
              {templateOptions.map((template) => <option key={template.slug} value={template.slug}>{template.name}</option>)}
            </select>
          </label>

          <label className="block">
            <span className="text-xs font-semibold text-elegant/50 uppercase tracking-wider">Partikel Cover</span>
            <select value={settings.particleMode || 'sparkle'} onChange={(e) => updateSetting('particleMode', e.target.value as any)} className="input-elegant mt-1">
          ...
          <label className="flex items-center justify-between gap-3 p-4 rounded-2xl bg-champagne/30">
            ...
            <input type="checkbox" checked={!!settings.cinematicMode} onChange={(e) => updateSetting('cinematicMode', e.target.checked)} />
          </label>

          <label className="flex items-center justify-between gap-3 p-4 rounded-2xl bg-champagne/30">
            <span>
              <b className="block text-elegant flex items-center gap-2"><Volume2 size={15} /> Voice Narration</b>
              <small className="text-elegant/50">Narasi suara otomatis dari browser.</small>
            </span>
            <input type="checkbox" checked={!!settings.voiceNarration} onChange={(e) => updateSetting('voiceNarration', e.target.checked)} />
          </label>

          <label className="block">
            <span className="text-xs font-semibold text-elegant/50 uppercase tracking-wider">Teks Narasi Suara</span>
            <textarea value={settings.voiceText || ''} onChange={(e) => updateSetting('voiceText', e.target.value)} className="input-elegant mt-1 min-h-[120px]" />
          </label>
        </section>
      </div>
    </div>
